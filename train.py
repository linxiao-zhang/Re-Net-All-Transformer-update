...
def train(args):
    ...
    while True:
        ...
        train_data_shuffle, s_history_shuffle, s_history_t_shuffle, o_history_shuffle, o_history_t_shuffle = shuffle(
            train_data, s_history, s_history_t,
        ...
        for batch_data, s_hist, s_hist_t, o_hist, o_hist_t in utils.make_batch2(train_data_shuffle, s_history_shuffle,
            ...
            loss_s = model(entity_tensor, batch_data, (s_hist, s_hist_t), (o_hist, o_hist_t), graph_dict, subject=True)
            loss_o = model(entity_tensor, batch_data, (s_hist, s_hist_t), (o_hist, o_hist_t), graph_dict, subject=False)
            loss = loss_s + loss_o
            loss.backward()
        ...
        if epoch % args.valid_every == 0 and epoch >= int(args.max_epochs / 2):
            ...
            model.init_history(entity_tensor, train_data, (s_history, s_history_t), (o_history, o_history_t),
                               valid_data,
            ...
            for i in range(len(valid_data)):
                ...
                with torch.no_grad():
                    ranks, loss = model.evaluate_filter(entity_tensor, batch_data, (s_hist, s_hist_t),
                                                        (o_hist, o_hist_t),
